    ...
    if (rank == padre)
    {
        ...
        for (int i = 0; i < dimension / cantidadProcesos; i++)
        {
            for (destino = 1; destino <= procesosActivos; destino++)
            {
                ...
                MPI_Scatter(&numeroFilas, 1, MPI_INT, &numeroFilas, etiquetaActividad, MPI_COMM_WORLD);
                MPI_Scatter(&cantFila, 1, MPI_INT, &numeroFilas, etiquetaActividad, MPI_COMM_WORLD);
                MPI_Scatter(&mA[numeroFilas][0], cantFila * dimension, MPI_DOUBLE, &numeroFilas, etiquetaActividad, MPI_COMM_WORLD);
                MPI_Scatter(&mB, dimension * dimension, MPI_DOUBLE, &numeroFilas, etiquetaActividad, MPI_COMM_WORLD);
